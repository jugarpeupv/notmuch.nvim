...
if match(bufname("%"), "^thread:") != -1
	...
	nnoremap <buffer> <silent> U <Cmd>call v:lua.require('notmuch.attach').get_urls_from_cursor_msg()<CR>
	nnoremap <buffer> <silent> <Tab> zj
	nnoremap <buffer> <silent> <S-Tab> zk
	nnoremap <buffer> <silent> <Enter> za
	nnoremap <buffer> <silent> a <Cmd>call v:lua.require('notmuch.attach').get_attachments_from_cursor_msg()<CR>
	nnoremap <buffer> <silent> r <Cmd>call v:lua.require('notmuch.refresh').refresh_thread_buffer()<CR>
	nnoremap <buffer> <silent> C <Cmd>call v:lua.require('notmuch.send').compose()<CR>
	nnoremap <buffer> <silent> R <Cmd>call v:lua.require('notmuch.send').reply()<CR>
